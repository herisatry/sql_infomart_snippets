...
SELECT mm.ticket_id
			...
			, EXTRACT(EPOCH
FROM (lead(mm.create_time,1,mm.create_time) over (PARTITION BY mm.ticket_id)) - mm.create_time) AS dt_next_diff_sec
			,mm.agent
			,mm.action_name
			,mm.state_id
			,mm.state_name
...
SELECT DISTINCT th2.ticket_id
		,th2.create_time
		,th2.owner_id
		, CASE WHEN th2.name = '%%in progress%%answer%%' THEN 'answer' WHEN th2.name = '%%answer%%new%%' THEN 'new' ELSE 'e' END AS my_state
