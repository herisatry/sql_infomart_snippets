...
,CASE WHEN (SUM(IRF.AFTER_CALL_WORK_COUNT)>0 and SUM(IRF.TALK_COUNT)>0 )THEN ROUND(SUM(IRF.AFTER_CALL_WORK_DURATION)/SUM(IRF.TALK_COUNT),2) ELSE '' END AS 'AVG After Call Work Time, sec'
...
,'' as "AHT_voice_outb"
,'' as "AHT_voice_inb"
,'' AS 'AVG After Call Work Time, sec'
