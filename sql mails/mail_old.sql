...
,COUNT(DISTINCT CASE WHEN T1.N_trans > 0 THEN T1.I_ID END) AS "# Transferred"
...
,count(distinct case when IT.INTERACTION_TYPE='Inbound' AND R2.RESOURCE_NAME='BasicEmailInbound.default.EmailInboundContactProcessed' then IRF.INTERACTION_ID end) as N_Inb
-- ,count(distinct case when IT.INTERACTION_TYPE='Outbound' then IRF.INTERACTION_ID end) as N_Outb
,count(distinct case when IT.INTERACTION_SUBTYPE='OutboundReply' and TD2.TECHNICAL_RESULT='Completed' AND R2.RESOURCE_NAME='BasicEmailInbound.default.EmailOutgoing' then IRF.INTERACTION_ID end) as N_Outb_Repl
-- ,count(distinct case when IT.INTERACTION_SUBTYPE='OutboundNew' then IRF.INTERACTION_ID end) as N_Outb_New
,count(distinct case when TD2.TECHNICAL_RESULT in ('Completed','Transferred') AND R2.RESOURCE_NAME='BasicEmailInbound.default.EmailInboundContactProcessed' then IRF.INTERACTION_ID end) as N_obr
,count(distinct case when TD2.TECHNICAL_RESULT in ('Transferred') AND R2.RESOURCE_NAME='BasicEmailInbound.default.EmailInboundContactProcessed' then IRF.INTERACTION_ID end) as N_trans
